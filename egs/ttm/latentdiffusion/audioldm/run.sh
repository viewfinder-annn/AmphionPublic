...
options=$(getopt -o c:n:s --long gpu:,config:,name:,stage:,resume:,resume_from_ckpt_path:,resume_type:,infer_expt_dir:,infer_output_dir:,text:,waveform:,file_list: -- "$@")
eval set -- "$options"
...
while true; do
  case $1 in
    ...
    # [Only for Inference] The waveform you want to refer when running inference.
    --waveform) shift; waveform=$1 ; shift ;;
    # [Only for Inference] Whether the text & waveform are the file containing the list of the text/waveform.
    --file_list) shift; file_list=$1 ; shift ;;
