...
def get_solver_from_xp(
    ...
    """
    logger.info(f"Loading solver from XP {xp.sig}. " f"Overrides used: {xp.argv}")
    cfg = xp.cfg
...
def init_seed_and_system(cfg):
    ...
    logger.debug(
        "Setting efficient attention backend to %s", cfg.efficient_attention_backend
    )
...
def main(cfg):
    ...
    log_name = (
        "%s.log.{rank}" % cfg.execute_only if cfg.execute_only else "solver.log.{rank}"
    )
