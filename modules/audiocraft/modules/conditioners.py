...
def nullify_condition(condition: ConditionType, dim: int = 1):
    ...
    assert (
        ...
        and isinstance(condition[0], torch.Tensor)
...
def nullify_joint_embed(embed: JointEmbedCondition) -> JointEmbedCondition:
    ...
    """
    null_wav, _ = nullify_condition(
        (embed.wav, torch.zeros_like(embed.wav)), dim=embed.wav.dim() - 1
    )
...
class Tokenizer:
    ...
    def __call__(
        self, texts: tp.List[tp.Optional[str]]
...
class WhiteSpaceTokenizer(Tokenizer):
    ...
    @tp.no_type_check
    def __call__(
        self, texts: tp.List[tp.Optional[str]], return_text: bool = False
    ) -> tp.Tuple[torch.Tensor, torch.Tensor]:
...
class LUTConditioner(TextConditioner):
    ...
    def __init__(
        self, n_bins: int, dim: int, output_dim: int, tokenizer: str, pad_idx: int = 0
    ):
...
class MelSpectrogramConditioner(WaveformConditioner):
    ...
    def __init__(self, output_dim: int, device: tp.Union[torch.device, str], **kwargs):
        self.cfg = types.SimpleNamespace(**kwargs)
    ...
    def _get_wav_embedding(self, x: WavCondition) -> torch.Tensor:
        ...
        # print(wav)
        mels = torch.stack(
            [self._get_mel_spectrogram(wav[i]).cpu().detach() for i in range(batch)]
        )
        return mels
...
class ChromaStemConditioner(WaveformConditioner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.autocast = TorchAutocast(
            enabled=device != "cpu", device_type=self.device, dtype=torch.float32
        )
        ...
        self.stem_indices = torch.LongTensor(
            [stem_sources.index("vocals"), stem_sources.index("other")]
        ).to(device)
        self.chroma = ChromaExtractor(
            sample_rate=sample_rate, n_chroma=n_chroma, radix2_exp=radix2_exp, **kwargs
        ).to(device)
        self.chroma_len = self._get_chroma_len()
        self.eval_wavs: tp.Optional[torch.Tensor] = self._load_eval_wavs(
            eval_wavs, n_eval_wavs
        )
        self.cache = None
    ...
    def _load_eval_wavs(
        self, path: tp.Optional[str], num_samples: int
        ...
        dataset: AudioDataset = AudioDataset.from_meta(
            ...
            min_audio_duration=self.duration,
            sample_rate=self.sample_rate,
            channels=1,
        )

        if len(dataset) > 0:
            eval_wavs = dataset.collater([dataset[i] for i in range(num_samples)]).to(
                self.device
            )
            logger.info(
                f"Using {len(eval_wavs)} evaluation wavs for chroma-stem conditioner"
            )
            return eval_wavs
        else:
    ...
    def _sample_eval_wavs(self, num_samples: int) -> torch.Tensor:
        ...
        assert (
            self.eval_wavs is not None
        ), "Cannot sample eval wavs as no eval wavs provided."
    ...
    def _get_stemmed_wav(self, wav: torch.Tensor, sample_rate: int) -> torch.Tensor:
        ...
        with self.autocast:
            ...
            stems = stems[
                :, self.stem_indices
            ]  # extract relevant stems for melody conditioning
    ...
    @torch.no_grad()
    def _get_full_chroma_for_cache(
        self, path: tp.Union[str, Path], x: WavCondition, idx: int
    ) -> torch.Tensor:
    ...
    def _extract_chroma_chunk(
        self, full_chroma: torch.Tensor, x: WavCondition, idx: int
    ) -> torch.Tensor:
    ...
    def _get_wav_embedding(self, x: WavCondition) -> torch.Tensor:
        ...
        if self.match_len_on_eval:
            ...
            if T > self.chroma_len:
                ...
                logger.debug(
                    f"Chroma was truncated to match length! ({T} -> {chroma.shape[1]})"
                )
                ...
                chroma = chroma[:, : self.chroma_len]
                logger.debug(
                    f"Chroma was repeated to match length! ({T} -> {chroma.shape[1]})"
                )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if autocast_dtype is None or device == "cpu":
            self.autocast = TorchAutocast(enabled=False)
            logger.warning(
                "JointEmbeddingConditioner has no autocast, this might lead to NaN."
            )
        else:
            dtype = getattr(torch, autocast_dtype)
            assert isinstance(dtype, torch.dtype)
            logger.info(
                f"JointEmbeddingConditioner will be evaluated with autocast as {autocast_dtype}."
            )
            self.autocast = TorchAutocast(
                enabled=True, device_type=self.device, dtype=dtype
            )
...
class CLAPEmbeddingConditioner(JointEmbeddingConditioner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ImportError(
                "Please install CLAP to use the CLAPEmbeddingConditioner: 'pip install laion_clap'"
            )
        warnings.warn(
    ...
    def _compute_text_embedding(self, text: tp.List[str]) -> torch.Tensor:
        ...
        """
        with torch.no_grad():
            embed = self.clap.get_text_embedding(
                text, tokenizer=self._tokenizer, use_tensor=True
            )
            return embed.view(embed.size(0), 1, embed.size(-1))

    def _get_text_embedding_for_cache(
        self, path: tp.Union[Path, str], x: JointEmbedCondition, idx: int
    ) -> torch.Tensor:
    ...
    def _preprocess_wav(
        self, wav: torch.Tensor, length: torch.Tensor, sample_rates: tp.List[int]
    ) -> torch.Tensor:
        ...
        if sample_rates is not None:
            ...
            for i, audio in enumerate(wav):
                sr = sample_rates[i]
                audio = convert_audio(
                    audio, from_rate=sr, to_rate=self.clap_sample_rate, to_channels=1
                )
                _wav.append(audio)
    ...
    def _compute_wav_embedding(
        ...
        with torch.no_grad():
            ...
            if T >= self.clap_max_frames:
                wav = wav.unfold(
                    -1, self.clap_max_frames, self.clap_stride
                )  # [B, F, T]
            else:
    ...
    def _get_wav_embedding_for_cache(
        self, path: tp.Union[str, Path], x: JointEmbedCondition, idx: int
    ) -> torch.Tensor:
        ...
        embed = self._compute_wav_embedding(
            wav, wav_len, [sr], reduce_mean=False
        )  # [B, F, D]
        return embed.squeeze(0)  # [F, D]

    def _extract_wav_embedding_chunk(
        self, full_embed: torch.Tensor, x: JointEmbedCondition, idx: int
    ) -> torch.Tensor:
        ...
        seek_time = x.seek_time[idx]
        seek_time = 0.0 if seek_time is None else seek_time
    ...
    def _get_text_embedding(self, x: JointEmbedCondition) -> torch.Tensor:
        ...
        no_nullified_cond = (
            x.wav.shape[-1] > 1
        )  # we don't want to read from cache when condition dropout
    ...
    def _get_wav_embedding(self, x: JointEmbedCondition) -> torch.Tensor:
        ...
        if self.wav_cache is not None and no_undefined_paths and no_nullified_cond:
            ...
            embed = self._compute_wav_embedding(
                x.wav, x.length, x.sample_rate, reduce_mean=True
            )
        if self.normalize:
            embed = torch.nn.functional.normalize(embed, p=2.0, dim=-1)
        return embed
...
def dropout_condition(
    sample: ConditioningAttributes, condition_type: str, condition: str
    ...
    def forward(
        ...
        for condition_type, ps in self.p.items():  # for condition types [text, wav]
            for (
                condition,
                p,
            ) in ps.items():  # for attributes of each type (e.g., [artist, genre])
...
class ConditioningProvider(nn.Module):
    ...
    def joint_embed_conditions(self):
        return [
            m.attribute
            for m in self.conditioners.values()
            if isinstance(m, JointEmbeddingConditioner)
        ]
    ...
    def text_conditions(self):
        return [
            k for k, v in self.conditioners.items() if isinstance(v, TextConditioner)
        ]
    ...
    def wav_conditions(self):
        return [
            ...
            if isinstance(v, WaveformConditioner)
        ]
    ...
    def _collate_joint_embeds(
        self, samples: tp.List[ConditioningAttributes]
    ) -> tp.Dict[str, JointEmbedCondition]:
        ...
        for sample in samples:
            for attribute in self.joint_embed_conditions:
                wav, text, length, sample_rate, path, seek_time = sample.joint_embed[
                    attribute
                ]
                assert wav.dim() == 3
                if channels == 0:
                    ...
                    assert channels == wav.size(
                        1
                    ), "not all audio has same number of channels in batch"
                assert (
                    wav.size(0) == 1
                ), "Expecting single-wav batch in the collate method"
        ...
        for attribute in self.joint_embed_conditions:
            ...
            stacked_wavs = einops.rearrange(
                stacked_wavs, "(c t) b -> b c t", c=channels
            )
