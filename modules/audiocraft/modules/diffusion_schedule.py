...
class MultiBandProcessor(SampleProcessor):
    ...
    def __init__(
        self,
        n_bands: int = 8,
        sample_rate: float = 24_000,
        num_samples: int = 10_000,
        power_std: tp.Union[float, tp.List[float], torch.Tensor] = 1.0,
    ):
        super().__init__()
    ...
    def project_sample(self, x: torch.Tensor):
        ...
        rescale = (
            self.target_std / self.std.clamp(min=1e-12)
        ) ** self.power_std  # same output size
...
class NoiseSchedule:
    ...
    def __init__(
        ...
        device="cuda",
        beta_exp: float = 1,
        repartition: str = "power",
        alpha_sigmoid: dict = {},
        n_bands: tp.Optional[int] = None,
    ...
    def get_alpha_bar(
        self, step: tp.Optional[tp.Union[int, torch.Tensor]] = None
    ) -> torch.Tensor:
    ...
    def get_training_item(
        self, x: torch.Tensor, tensor_step: bool = False
    ) -> TrainingItem:
        ...
        noisy = (alpha_bar.sqrt() / self.rescale) * x + (
            1 - alpha_bar
        ).sqrt() * noise * self.noise_scale
        return TrainingItem(noisy, noise, step)

    def generate(
        self,
        model: torch.nn.Module,
        initial: tp.Optional[torch.Tensor] = None,
        ...
        for step in range(self.num_steps)[::-1]:
            ...
            previous = (
                current - (1 - alpha) / (1 - alpha_bar).sqrt() * estimate
            ) / alpha.sqrt()
    ...
    def generate_subsampled(
        self,
        model: torch.nn.Module,
        initial: torch.Tensor,
        step_list: tp.Optional[list] = None,
        condition: tp.Optional[torch.Tensor] = None,
        return_list: bool = False,
    ):
        ...
        alpha_bar = self.get_alpha_bar(step=self.num_steps - 1)
        alpha_bars_subsampled = (
            (1 - self.betas).cumprod(dim=0)[list(reversed(step_list))].cpu()
        )
        ...
        for idx, step in enumerate(step_list[:-1]):
            with torch.no_grad():
                estimate = (
                    model(current, step, condition=condition).sample * self.noise_scale
                )
