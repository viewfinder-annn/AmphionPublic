...
class SampleManager:
    ...
    def _get_sample_id(
        ...
        if conditions:
            ...
            cond_str = "-".join(
                [f"{key}={slugify(value)}" for key, value in sorted(conditions.items())]
            )
            cond_str = cond_str[
                :100
            ]  # some raw text might be too long to be a valid filename
    ...
    def _store_audio(
        self, wav: torch.Tensor, stem_path: Path, overwrite: bool = False
    ) -> Path:
        ...
        existing_paths = [
            path
            for path in stem_path.parent.glob(stem_path.stem + ".*")
            if path.suffix != ".json"
        ]
        exists = len(existing_paths) > 0
        if exists and overwrite:
            logger.warning(
                f"Overwriting existing audio file with stem path {stem_path}"
            )
        elif exists:
    ...
    def add_sample(
        ...
        if prompt_wav is not None:
            prompt_id = (
                sample_id
                if reuse_id
                else self._get_tensor_id(prompt_wav.sum(0, keepdim=True))
            )
            prompt_duration = prompt_wav.shape[-1] / self.xp.cfg.sample_rate
            prompt_path = self._store_audio(
                prompt_wav, self.base_folder / str(epoch) / "prompt" / prompt_id
            )
        ...
        if ground_truth_wav is not None:
            ground_truth_id = (
                sample_id
                if reuse_id
                else self._get_tensor_id(ground_truth_wav.sum(0, keepdim=True))
            )
            ground_truth_duration = ground_truth_wav.shape[-1] / self.xp.cfg.sample_rate
            ground_truth_path = self._store_audio(
                ground_truth_wav, self.base_folder / "reference" / ground_truth_id
            )
            ground_truth = ReferenceSample(
                ground_truth_id, str(ground_truth_path), ground_truth_duration
            )
        sample_path = self._store_audio(
            sample_wav, self.base_folder / str(epoch) / sample_id, overwrite=True
        )
    ...
    def add_samples(
        ...
        for idx, wav in enumerate(samples_wavs):
            ...
            samples.append(
                self.add_sample(
                    wav, epoch, idx, conditions, prompt_wav, gt_wav, generation_args
                )
            )
        return samples
    ...
    def get_samples(
        ...
        if max_epoch >= 0:
            samples_epoch = max(
                sample.epoch for sample in self.samples if sample.epoch <= max_epoch
            )
        else:
...
def _match_stable_samples(
    ...
    stable_samples = {
        id: [xp.get(id) for xp in stable_samples_per_xp] for id in stable_ids
    }
    ...
    return {
        id: tp.cast(tp.List[Sample], samples)
        for id, samples in stable_samples.items()
...
def _match_unstable_samples(
    ...
    return {
        f"noinput_{i}": [samples[i] for samples in unstable_samples_per_xp]
        for i in range(min_len)
    }


def get_samples_for_xps(
    xps: tp.List[dora.XP], **kwargs
) -> tp.Dict[str, tp.List[Sample]]:
