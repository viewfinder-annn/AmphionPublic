...
class ScaleDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        assert inner_kernel_sizes is None or len(inner_kernel_sizes) == len(
            downsample_scales
        )
        ...
        for i, downsample_scale in enumerate(downsample_scales):
            ...
            self.convs.append(
                NormConv1d(
                    ...
                    out_chs,
                    kernel_size=(
                        inner_kernel_sizes[i]
                        if inner_kernel_sizes
                        else default_kernel_size
                    ),
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.discriminators = nn.ModuleList(
            [
                ScaleDiscriminator(in_channels, out_channels, norm=norm, **kwargs)
                for norm in scale_norms
            ]
        )
        self.downsample = nn.AvgPool1d(
            downsample_factor * 2, downsample_factor, padding=downsample_factor
        )
