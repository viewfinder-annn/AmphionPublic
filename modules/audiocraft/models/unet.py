...
class DecoderLayer(nn.Module):
    def __init__(
        ...
        self.res_blocks = nn.Sequential(
            *[
                ResBlock(
                    chin, norm_groups=norm_groups, dilation=2**idx, dropout=dropout
                )
...
class EncoderLayer(nn.Module):
    def __init__(
        ...
        self.res_blocks = nn.Sequential(
            *[
                ResBlock(
                    chout, norm_groups=norm_groups, dilation=2**idx, dropout=dropout
                )
                for idx in range(res_blocks)
            ]
        )
    ...
    def forward(
        ...
        if type(step) is torch.Tensor:
            ...
            step_tensor = torch.tensor(
                [step], device=x.device, dtype=torch.long
            ).expand(bs)

        for idx, encoder in enumerate(self.encoders):
            z = encoder(z)
            if idx == 0:
                z = z + self.embedding(step_tensor).view(bs, -1, *view_args).expand_as(
                    z
                )
        ...
        if self.use_codec:  # insert condition in the bottleneck
            ...
            assert condition_emb.size(-1) <= 2 * z.size(
                -1
            ...
            if not self.cross_attention:
                ...
                pos_emb = create_sin_embedding(
                    positions, C, max_period=10_000, dtype=cross_attention_src.dtype
                )
                cross_attention_src = cross_attention_src + pos_emb
        if self.use_transformer:
            z = self.transformer(
                z.permute(0, 2, 1), cross_attention_src=cross_attention_src
            ).permute(0, 2, 1)
        else:
