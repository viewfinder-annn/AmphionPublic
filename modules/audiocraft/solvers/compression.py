...
class CompressionSolver(base.StandardSolver):
    ...
    def load_from_pretrained(self, name: str) -> dict:
        ...
        if isinstance(model, models.DAC):
            ...
            self.logger.warning(
                "Trying to automatically convert a HuggingFace model "
            ...
            for k, v in state.items():
                if (
                    ...
                    layer = int(k.split(".")[2])
                    if isinstance(
                        model.model.decoder.layers[layer].conv, torch.nn.ConvTranspose1d
                    ):
...
def evaluate_audio_reconstruction(
    y_pred: torch.Tensor, y: torch.Tensor, cfg: omegaconf.DictConfig
) -> dict:
