...
class MusicGenSolver(base.StandardSolver):
    ...
    def _prepare_tokens_and_attributes(
        ...
        if self._cached_batch_writer is not None and self.current_stage == "train":
            ...
            for info, one_audio_tokens in zip(infos, audio_tokens):
                ...
                if isinstance(info, MusicInfo):
                    assert (
                        not info.joint_embed
                    ), "joint_embed and cache not supported yet."
                    info.self_wav = None
    ...
    def run_generate_step(
        ...
        **generation_params,
        ...
        if prompt_duration is None:
            ...
            assert (
                prompt_duration < gen_duration
            ), "Prompt duration must be lower than target generation duration"
    ...
    def run_epoch(self):
        if self.cfg.cache.write:
            if (
                (self.epoch - 1) % self.cfg.cache.write_num_shards
            ) != self.cfg.cache.write_shard:
                return
        super().run_epoch()
    ...
    def evaluate_audio_generation(self) -> dict:
        ...
        if self.cfg.evaluate.metrics.text_consistency:
            text_consistency = builders.get_text_consistency(
                self.cfg.metrics.text_consistency
            ).to(self.device)
            should_run_eval = True
        if self.cfg.evaluate.metrics.chroma_cosine:
            chroma_cosine = builders.get_chroma_cosine_similarity(
                self.cfg.metrics.chroma_cosine
            ).to(self.device)
            # if we have predefind wavs for chroma we should purge them for computing the cosine metric
            has_predefined_eval_chromas = (
                "self_wav" in self.model.condition_provider.conditioners
                and self.model.condition_provider.conditioners[
                    "self_wav"
                ].has_eval_wavs()
            )
            if has_predefined_eval_chromas:
                warn_once(
                    self.logger,
                    "Attempting to run cosine eval for config with pre-defined eval chromas! "
        ...
        if should_run_eval:
            ...
            for idx, batch in enumerate(lp):
                ...
                gen_outputs = self.run_generate_step(
                    batch, gen_duration=target_duration, **self.generation_params
                )
                ...
                y_pred = torch.stack(
                    [normalize_audio(w, **normalize_kwargs) for w in y_pred], dim=0
                ).cpu()
                y = audio.cpu()  # should already be on CPU but just in case
                sizes = torch.tensor(
                    [m.n_frames for m in meta]
                ...
                if chroma_cosine is not None:
                    ...
                    if eval_chroma_wavs is not None:
                        self.model.condition_provider.conditioners[
                            "self_wav"
                        ].reset_eval_wavs(eval_chroma_wavs)
